...
describe 'Admin Invoices Show Page' do
  ...
  it "shows the total revenue with bulk discounts for this invoice" do
    ...
    ii_1 = InvoiceItem.create!(invoice_id: invoice_1.id, item_id: item_1.id, quantity: 13, unit_price: 10, status: 1)
    ...
    ii_7 = InvoiceItem.create!(invoice_id: invoice_1.id, item_id: item_7.id, quantity: 1, unit_price: 3, status: 1)
    ii_8 = InvoiceItem.create!(invoice_id: invoice_1.id, item_id: item_8.id, quantity: 1, unit_price: 5, status: 1)
    ...
    within('#total_revenue_with_bulk_discounts') do
      expect(page).to have_content("Total Revenue Including Bulk Discounts: #{invoice_1.total_revenue_with_bulk_discounts}")
    end
  end
end
